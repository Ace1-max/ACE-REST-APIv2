...
exports.config = {
    name: 'gensin',
    author: 'AceGerome',
    description: 'Randomly retrieves a Genshin Impact image',
    method: 'get',
    category: 'anime',
    link: ['/gensin']
};
...
exports.initialize = async function ({ req, res }) {
    try {
        const gensinImages = [
            ...
            "https://i.pinimg.com/originals/ee/91/3c/ee913c0b8d4e2b5d704bac983285c800.jpg",
        ...
        const randomImage = gensinImages[Math.floor(Math.random() * gensinImages.length)];

        const response = await axios.get(randomImage, { responseType: 'arraybuffer' });

        res.setHeader('Content-Type', 'image/jpeg'); 
        res.setHeader('Content-Disposition', 'inline; filename="gensin.jpg"');

        res.send(Buffer.from(response.data));
    } catch (error) {
        console.error("Error fetching Genshin image:", error.message);
        res.status(500).json({
            status: false,
            creator: this.config.author,
            message: 'Failed to retrieve a Genshin image.'
        });
    }
};
